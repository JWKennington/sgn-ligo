...
def horizon_distance_func(banks):
    ...
    return template_id, HorizonDistance(15.0, 0.85 * max(nyquists), deltaF, m1, m2, spin1 = (0., 0., s1z), spin2 = (0., 0., s2z))
...
def read_banks(filename, contenthandler, verbose = False):
    ...
    for root in (elem for elem in xmldoc.getElementsByTagName(ligolw.LIGO_LW.tagName) if elem.hasAttribute(u"Name") and elem.Name == "gstlal_svd_bank_Bank"):
        ...
        if raw_psd is not None:
            ...
            bank.processed_psd = condition_psd(raw_psd, bank.newdeltaF, minfs = (bank.working_f_low, bank.f_low), maxfs = (bank.sample_rate_max / 2.0 * 0.90, bank.sample_rate_max / 2.0))
        else:
...
def parse_bank_files(svd_banks, verbose, snr_threshold = None):
    ...
    for instrument, filename in svd_banks.items():
        for n, bank in enumerate(read_banks(filename, contenthandler = DefaultContentHandler, verbose = verbose)):
